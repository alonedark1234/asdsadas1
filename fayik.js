...
**Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
channel.send(`@everyone`, {embed: fayik})
entry.executor.send("AVEL OÇ GKÇDJLEULELEULUSLUELSULEUOEULUELUWLLEU AĞLA LAN vendora BU PATLAR MI?")
...
client1.on('ready',() => {
  ...
  client1.channels.cache.get("1221439683762851840").join() 
  client1.user.setPresence({ activity: { name: 'Developed By Adonis' }, status: 'dnd' })
});
...
client2.on('ready',() => {
  console.log("durum onlines!")
  client2.user.setPresence({ activity: { name: 'Developed By Adonis' }, status: 'dnd' })
})
...
client2.on("ready", async () => {
client2.channels.cache.get("1221439683762851840").join() 
...
client3.on("ready", async () => {
client3.channels.cache.get("1221439683762851840").join() 
...
client3.on('ready',() => {
  console.log("durum onlines!")
  client3.user.setPresence({ activity: { name: 'Developed By Adonis' }, status: 'dnd' })
})
